...
* 1) define the data buffer
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_buffer.
             INCLUDE TYPE   ztbooking_hvam AS data.
...
CLASS lcl_handler IMPLEMENTATION.
  METHOD modify.
    ...
    IF roots_to_create IS NOT INITIAL.
      ...
      SELECT SINGLE MAX( booking ) FROM ztbooking_hvam INTO @DATA(lv_max_booking).
    ENDIF.
    ...
    IF roots_to_update IS NOT INITIAL.
      LOOP AT roots_to_update INTO DATA(ls_update).
        ...
        IF sy-subrc <> 0.
          ...
          SELECT SINGLE * FROM ztbooking_hvam WHERE booking = @ls_update-booking INTO @DATA(ls_db).
  ...
  METHOD read.
    LOOP AT it_booking_key INTO DATA(ls_booking_key).
      ...
      ELSE.
        SELECT SINGLE * FROM ztbooking_hvam WHERE booking = @ls_booking_key-booking INTO @DATA(ls_db).
        IF sy-subrc = 0.
